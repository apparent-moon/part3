...
public class SavingBank extends Bank {
    ...
    public void withdraw(SavingAccount account) throws Exception {
        ...
        try {
            ...
            // 목표금액보다 잔액이 작으면
            if (accountGoalAmount.compareTo(accountBalance) == 1) {
                System.out.println("목표금액을 달성해야 출금이 가능합니다. 처음 화면으로 돌아갑니다.\n");
                System.out.println("========================================================================");
                System.out.printf("계좌종류: %s | 계좌번호: %s | 계좌주명: %s | 잔액: %s원 | 목표금액 : %s원\n", account.getCategory(), account.getAccNo(), account.getOwner(), df.format(accountBalance), df.format(accountGoalAmount), df.format(account.getGoalAmount()));
                System.out.println("========================================================================");
            } else {
